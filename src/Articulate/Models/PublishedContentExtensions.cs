...
namespace Articulate.Models
{
    ...
    {
        private static readonly Regex _markdownImageRegex = new(
            @"!\[.*?\]\((.*?)\)",
            RegexOptions.Compiled,
            TimeSpan.FromMilliseconds(250));

        [Obsolete(
            "Prefer an extension method from Umbraco.Extensions.FriendlyImageCropperTemplateExtensions.GetCropUrl()")]
        public static string GetArticulateCropUrl(
        ...
        public static string GetFirstMarkdownImageUrl(this PostModel model)
        {
            ...
            Match match = _markdownImageRegex.Match(markdown);
