...
namespace Articulate.Services
{
        ...
        INotificationAsyncHandler<UmbracoApplicationStartedNotification>
    {
        ...
        public Task HandleAsync(
            ...
            EnsureBootstrapAsync(cancellationToken);
        ...
        private async Task EnsureBootstrapAsync(CancellationToken cancellationToken)
        {
            ...
            {
                logger.LogWarning("Skipping Articulate dev automation bootstrap: no API user display name was configured.");
                return;
            }

            await using AsyncServiceScope scope = scopeFactory.CreateAsyncScope();
            IUserService userService = scope.ServiceProvider.GetRequiredService<IUserService>();
            IBackOfficeUserStore userStore = scope.ServiceProvider.GetRequiredService<IBackOfficeUserStore>();
            ICoreBackOfficeUserManager coreUserManager = scope.ServiceProvider.GetRequiredService<ICoreBackOfficeUserManager>();
            ...
            IUserGroupService userGroupService = scope.ServiceProvider.GetRequiredService<IUserGroupService>();
            ...
            IReadOnlyList<IReadOnlyUserGroup> requiredGroups =
                await ResolveGroupsAsync(userGroupService, settings.UserGroupAliases, cancellationToken);
            if (requiredGroups.Count == 0)
            {
                return;
            }

            IUser? user = clientBoundUser;
            if (user is not null)
            {
                logger.LogInformation(
                    "Articulate dev automation client '{ClientId}' is already bound to user '{UserName}'.",
                    settings.ClientId,
                    user.Username);
            }
            else
            ...
            if (user is null)
            {
                IdentityCreationResult? createResult = await CreateApiUserAsync(
                    coreUserManager,
                    settings,
                    requiredGroups);

                if (createResult is null || createResult.Succeded is false)
                {
                    logger.LogWarning(
            ...
            else if (user.Kind != UserKind.Api)
            {
                logger.LogWarning(
            ...
            {
                await backOfficeApplicationManager.EnsureBackOfficeClientCredentialsApplicationAsync(
                    settings.ClientId,
