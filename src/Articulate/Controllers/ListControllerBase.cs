...
namespace Articulate.Controllers
{
        ...
        : RenderController(logger, compositeViewEngine, umbracoContextAccessor)
    {
        ...
        protected IVariationContextAccessor VariationContextAccessor { get; } = variationContextAccessor;

        /// <summary>
        /// Gets a paged list view for a given posts by author/tags/categories model
        /// </summary>
        protected IActionResult GetPagedListView(IMasterModel masterModel, IPublishedContent pageNode,
            IEnumerable<IPublishedContent> listItems, long totalPosts, int? p)
        {
            if (masterModel == null)
            {
        ...
        protected bool GetPagerModel(IMasterModel masterModel, long totalPosts, int? p, out PagerModel pager)
        {
            ...
            foreach (var key in Request.Query.Keys)
            {
                ...
                if (Request.Query.TryGetValue(key, out var val))
                {
