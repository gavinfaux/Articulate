...
{
    [ManagementApi(ArticulateEnum.ManagementApi.Template)]
    [ApiVersion("1.0")]
    ...
    public class TemplateController(
        ...
        ILogger<TemplateController> logger) : ManagementApiControllerBase
    {
        [HttpPost("copy")]
        ...
        public async Task<IActionResult> Clone(ThemeCopyModel model)
        {
            if (string.IsNullOrWhiteSpace(model.NewThemeName) ||
                model.NewThemeName.IndexOfAny(Path.GetInvalidFileNameChars()) >= 0)
            {
                return BadRequest(new ProblemDetails
                {
                    Title = "Invalid Theme Name", Detail = "The new theme name contains invalid characters."
                });
            }
            ...
            if (Directory.Exists(destinationPhysicalPath))
            {
                return Conflict(new ProblemDetails
                {
                    ...
                    Detail = $"A theme with the name '{model.NewThemeName}' already exists."
                });
            }
            ...
            if (!Directory.Exists(sourcePhysicalPath))
            {
                ...
                if (!Directory.Exists(sourcePhysicalPath))
                {
                    return NotFound(new ProblemDetails
                    {
                        ...
                        Detail = $"The source theme '{model.ThemeName}' could not be found."
                    });
                }
            }
            ...
            catch (Exception ex)
            {
                logger.LogError(ex,
                    ...
                    model.ThemeName, model.NewThemeName);
                return Problem("An unexpected error occurred during the theme copy operation.",
        ...
        public IActionResult GetDefaultThemes()
        {
            ...
            catch (Exception ex)
            {
                ...
                return Problem("An error occurred while retrieving default themes.",
                    statusCode: StatusCodes.Status500InternalServerError);
            }
        }
