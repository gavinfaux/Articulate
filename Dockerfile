...
# UMBRACO_CMS_VERSION is required; build.cs reads the floor from Directory.Packages.props
...
# build.cs reads it from Directory.Packages.props and passes as --build-arg.
...
RUN set -eux; \
    ...
    if [ -z "${UMBRACO_CMS_VERSION}" ]; then \
      echo 'ERROR: UMBRACO_CMS_VERSION build-arg is required. Pass --build-arg UMBRACO_CMS_VERSION=<range>, e.g. [17.5.3,18.0.0) for v17 or [18.0.2,19.0.0) for v18. build.cs reads the floor from Directory.Packages.props.'; \
      exit 1; \
    fi; \
