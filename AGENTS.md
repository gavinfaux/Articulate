...
- **`ENABLE_CLIENT_BUILD`** (environment variable):
  - Default: `false` locally, `true` automatically in CI.
  - Set to `true` to run the pnpm client build (e.g., `export ENABLE_CLIENT_BUILD=true` or `set ENABLE_CLIENT_BUILD=true`).
...
- Example with `fnm` (optional):
  ...
  - Restart your shell, then in repo root: `fnm use` (respects `.nvmrc`), then `corepack enable && corepack prepare pnpm@10.20.0 --activate`.
