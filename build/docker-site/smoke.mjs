...
async function verifyThemeInHtml(base, expectedTheme, timeoutSec) {
  const marker = getThemeCssMarker(expectedTheme);
  await poll(async () => {
    try {
      const res = await request(`${base}/`, { timeout: 15_000 });
      if (res.status !== 200) return false;
      return res.body.includes(marker);
    } catch { return false; }
...
async function main() {
  ...
  if (!['publish', 'confirm', 'smoke', 'theme'].includes(mode)) {
  ...
  if (mode === 'smoke') {
    console.log('[1/1] Waiting for production root to return 200');
  ...
  // --- theme mode -----------------------------------------------------------
  if (mode === 'theme') {
    console.log('[3/7] Reading current document');
    ...
    console.log('[4/7] Verifying current theme renders');
  ...
  // --- publish mode ---------------------------------------------------------
  const steps = mode === 'publish' && !noDescendants ? 8 : 5;

  if (noDescendants) {
    console.log(`[3/${steps}] Publishing root only`);
    await publishRoot(base, token, rootId);
  } else {
    console.log(`[3/${steps}] Publishing root before descendants`);
    await publishRoot(base, token, rootId);

    console.log(`[4/${steps}] Waiting for root cache`);
    await reloadCache(base, token);
    await waitForRoot(base, timeoutSec);

    console.log(`[5/${steps}] Publishing root children with descendants`);
    const data = await jsonGet(`${base}/umbraco/management/api/v1/tree/document/children?parentId=${rootId}&skip=0&take=100`, token);
    ...
    console.log(`[6/${steps}] Publishing root with descendants`);
    await publishWithDescendants(base, token, rootId, 'root', timeoutSec);
  }

  console.log(`[${steps - 1}/${steps}] Reloading published cache`);
  await reloadCache(base, token);

  console.log(`[${steps}/${steps}] Waiting for public root`);
  await waitForRoot(base, timeoutSec);
