...
{
    var cms = Env.GetUmbracoCmsPackageVersionRange(lane);
    await Run("docker", new[]
    {
        ...
        "--build-arg", $"TINYMCE_UMBRACO_PACKAGE_VERSION={Env.Get("TINYMCE_UMBRACO_PACKAGE_VERSION", Env.GetTinyMceUmbracoPackageVersionRange(lane))}",
        "."
    }, cwd: Env.Repo);
    return 0;
}
...
async Task<string?> ResolvePackageVersion(string lane)
{
    ...
    var baseVersion = File.ReadAllText(Path.Combine(Env.Repo, "version-v18.txt")).Trim();
    if (string.IsNullOrWhiteSpace(baseVersion))
        throw new InvalidOperationException("version-v18.txt is empty.");
...
static class Env
{
    ...
    // Resolve an MSBuild property from Directory.Build.props / Directory.Packages.props for the given lane.
    // Single source of truth: bumping a version in CPM updates both packable builds and the docker site.
    // .sln files don't accept -getProperty (MSB1063), so point at a packable project directly.
    static string GetMsbuildProperty(string name, string lane)
    {
        var project = Path.Combine(Repo, "src", "Articulate.Web", "Articulate.Web.csproj");
        var psi = new ProcessStartInfo("dotnet",
            $"msbuild {project} -getProperty:{name} -p:ArticulatePackageLane={lane}")
        {
            RedirectStandardOutput = true,
            UseShellExecute = false,
            CreateNoWindow = true,
        };
        using var p = Process.Start(psi) ?? throw new InvalidOperationException("Could not start dotnet msbuild.");
        var output = p.StandardOutput.ReadToEnd().Trim();
        p.WaitForExit();
        if (p.ExitCode != 0 || string.IsNullOrEmpty(output))
            throw new InvalidOperationException(
                $"dotnet msbuild -getProperty:{name} failed (exit {p.ExitCode}). " +
                $"Lane '{lane}' must be v17 or v18 and the property must be defined.");
        return output;
    }

    public static string GetUmbracoCmsPackageVersionRange(string lane) => GetMsbuildProperty("UmbracoCmsPackageVersion", lane);

    public static string GetTinyMceUmbracoPackageVersionRange(string lane) => GetMsbuildProperty("TinyMceUmbracoPackageVersion", lane);
    ...
    public static readonly (string Key, Func<string, string, string, string> Value)[] LaneDefaults =
    {
        ...
        ("UMBRACO_CMS_VERSION",                                  (l, _, _) => Env.GetUmbracoCmsPackageVersionRange(l)),
        ("TINYMCE_UMBRACO_PACKAGE_VERSION",                      (l, _, _) => Env.GetTinyMceUmbracoPackageVersionRange(l)),
