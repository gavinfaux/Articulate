...
//
// Conventions:
//   - Restore + build + test + pack for the v17 target.
...
async Task<int> ClientAsync(Opts o)
{
    ...
    var laneDir = Path.Combine(workspace, "v17");
...
void DeleteClientBuildStamps(string clientAssetsDir)
{
    ...
    foreach (var stamp in Directory.EnumerateFiles(clientAssetsDir, "BackofficeClient_v17*.stamp"))
        DeleteFile(stamp);
}
